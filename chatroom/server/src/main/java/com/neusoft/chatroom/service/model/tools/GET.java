...
import com.neusoft.chatroom.api.service.ObjSerializeAndDeserialize;
...
public class GET extends Thread {// 获取客户端的请求，并处理该请求，响应客户端
	...
	public GET(Socket stemp, Socket sendtemp, Userinfo u, LiGong xiaoli,
			Thread t1) {
		s = stemp;
		sends = sendtemp;
		user = u;
		...
		try {
			while (true) {
				...
				if (StateCode.LOGIN.equals(stateCode)) {
					...
					ui.setStateCode(blogin ? StateCode.SUCCESS
							: StateCode.ERROR);
					ObjSerializeAndDeserialize.SerializeObj(
							s.getOutputStream(), ui);
				...
				// String s =
				// StringEdit.hechengUser(StateCode.SUCCESS,user,user.getIp());
				// p.println(s);
				// }
				...
				// l = us.selectAll();
				// String alluser =
				// StringEdit.hechengUserS(StateCode.SUCCESS,l);
				// p.println(alluser);
				// }
				...
				//
				// String alluser = StringEdit.hechengUserS2(StateCode.SUCCESS,
				// l);
				// p.println(alluser);
				// }
